...
将`根目录\themes\LoveIt\exampleSite`路径下的`config.toml`文件复制到[根目录](https://so.csdn.net/so/search?q=根目录&spm=1001.2101.3001.7020)下，覆盖掉根目录下的`config.toml`文件。然后，我们在notepad++中打开并对其作一定的修改就可以直接使用，具体可以修改的内容如下：
...
将根目录`\archetypes`下的`default.md`修改如下：
